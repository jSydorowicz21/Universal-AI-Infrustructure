...
export function lowerAgentIntent(adapterId: FirstPartyAdapterId, intent: AgentIntent): LoweredIntent {
  ...
  if (adapterId === "codex") return { adapterId, nativeName: intent.id, config: { agentFile: `.codex/agents/${intent.id}.toml`, agent: { name: intent.id, description: intent.instructions, developer_instructions: intent.instructions, ...(intent.model ? { model: intent.model } : {}), ...(intent.reasoning ? { model_reasoning_effort: intent.reasoning } : {}) } }, losses: ["no implicit installation-time spawn; delegation requires explicit user request or AGENTS.md/skill instruction", ...(intent.model ? ["model selection is per-agent-file, not per-invocation"] : []), ...(intent.reasoning ? ["reasoning effort is adapter-specific (model_reasoning_effort)"] : [])] };
