...
4. **System overlay** — place the harness-root system files (each `existsSync`-guarded — never clobber a populated harness):
   ...
   - Identity rendering is deferred until the full runtime and USER scaffold exist. The verification step runs `RenderIdentity.ts` across the owned model-facing surfaces and verifies that every identity/version token is gone from those surfaces.
...
6. **LinkUser** — `bun Tools/LinkUser.ts` (dry-run first, then `--apply`) → losslessly migrate and symlink both `LIFEOS/USER` and `LIFEOS/MEMORY` into the canonical private data root. Existing destination files displaced by live state are preserved; both link contracts are verified.
...
8.6. **Render identity (Core)** — preview `bun Tools/RenderIdentity.ts --config-root <configRoot> --config-dir <dataRoot>`, then run it with `--apply`. It derives names and voice IDs from installed settings plus canonical USER identity files, rewrites only payload-owned deployed templates, preserves unrelated/custom files, and exits non-zero if any identity/version placeholder survives those owned surfaces.
9. **Verify (four evidence classes)** — (a) `RenderIdentity.ts --apply` reports `verification.passed: true`; (b) the config tree resolves and the activated identity `@`-imports load; (c) IF the user opted into `hooks`, a probe session shows the mode banner / context injection fire. If hooks were declined, skip (c) and surface the caveat plainly: the constitutional mode banner and memory/voice loop are hook-enforced, so without hooks LifeOS Core installs but runs un-bannered and un-augmented — recommend hooks unless there's a reason to decline; (d) the **launch command is wired** — inspect the shell profile for the `lifeos` function/alias (or confirm the user was handed the manual one-line launch). Without it, Core installed but launches un-constituted — say so plainly. Report what was checked; never claim a hook fired or an import resolved without evidence.
