...
function resolveInterceptorBin(): string {
  ...
  if (found.exitCode !== 0 || bin.length === 0) {
    console.error("interceptor CLI not found on PATH. It is required only for Interceptor-based Claude Design verification; on Codex, use the native chrome/browser plugin lane for normal web UI checks.");
    process.exit(127);
  }
  return bin;
}
