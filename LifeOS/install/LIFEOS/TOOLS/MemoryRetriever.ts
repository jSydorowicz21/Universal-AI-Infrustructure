...
import { resolveConfigRoot, resolveDataRoot, resolveHome, resolveLifeosRoot } from "../UNIVERSAL/platform";
...
const USER_ROOT = path.join(resolveDataRoot(process.env), "USER");
...
const MEMORY_FILES: ReadonlyArray<{ path: string; title: string }> = [
  { path: path.join(USER_ROOT, "PRINCIPAL", "PRINCIPAL_MEMORY.md"), title: "Principal Memory" },
  { path: path.join(USER_ROOT, "DIGITAL_ASSISTANT", "DA_MEMORY.md"), title: "DA Memory" },
];
...
function formatRelevantBlock(results: RelevantResultItem[]): string {
  ...
  for (const r of results) {
    const shortPath = r.path.replace(CONFIG_ROOT + path.sep, "");
    lines.push("");
